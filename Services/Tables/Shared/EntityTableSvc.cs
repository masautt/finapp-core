...
public abstract class EntityTableSvc<TEntity>(TableEntityRepo entityRepo) : IEntityTableSvc<TEntity>
    where TEntity : class
{
    private readonly CommonTableSvc<TEntity> _commonSvc = new(entityRepo.CommonRepo);
    private readonly DateTableSvc<TEntity> _dateSvc = new(entityRepo.DateRepo);
