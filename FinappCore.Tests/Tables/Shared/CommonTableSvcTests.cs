...
public class CommonTableSvcTests
{
    private readonly CommonSvcWrapper _svc;

    public CommonTableSvcTests()
    {
        var provider = TestServiceInitializer.GetServiceProvider();
        if (provider == null)
            throw new InvalidOperationException("Service provider not initialized.");

        var commonRepo = provider.GetRequiredService<TableCommonRepo>();
...
public class CommonSvcWrapper(TableCommonRepo commonRepo) : CommonTableSvc<CarTableDto>(commonRepo);
